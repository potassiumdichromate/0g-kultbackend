...
| Save lifecycle: validation → (optional TEE verification) → encode → compress → upload to 0G → store rootHash → cache | `save-service` + `verification-service` | This is 0G-specific, security-sensitive infrastructure code — the exact code that was already duplicated twice; a game should never write it a third time |
| Security/anti-tamper for the save path | `save-service` (sync structural) + `verification-service` (async semantic/anti-cheat) | Wallet-from-JWT-never-body, server-computed saveIndex, schema validation — see `architecture/09-security-model.md` |
...
|---|---|---|---|
| **Game Adapters** (`zerodash-adapter`, `warzone-adapter`) | One bespoke service per game, hardcoded to that game's specific public endpoint shapes | A single generic, config-driven sync worker (or: nothing at all, once a game migrates to `save-service`) | Acknowledged technical debt — see `architecture/08-migration-roadmap.md`. These are explicitly **bridge/compatibility shims**, not a permanent architectural pattern. They retire per-game the moment that game migrates. |
...
| **Achievement/Reward criteria** | One hardcoded rule each (`first_save`, a ZeroDash-coins-to-Warzone-item threshold), written as TypeScript, not data | Criteria stored as data (the `Achievement.criteria` / `GameMetadata` Json fields already exist for this) and evaluated by one generic rule matcher | Schema is platform-centric already; the *evaluation code* is still one bespoke function per rule. Flagged for a follow-up refactor, not done this round. |
| **Per-game save schema registration** | `ZeroDashSaveDataSchema`/`WarzoneSaveDataSchema` live in a static code map in `shared/dto` — adding game #101 means editing and redeploying a shared package | Schema registration as a self-service platform capability (e.g. a JSON-schema column on `GameMetadata` a game owner submits, no shared-code edit) | Not built. The current approach is fine at 2 games, wrong at 100 — exactly the kind of thing this vision doc exists to flag before it calcifies. |
